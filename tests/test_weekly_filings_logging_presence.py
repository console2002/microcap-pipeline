...
def _make_cfg(tmp_path) -> dict:
    ...
    cfg.setdefault("Workers", {}).setdefault("EDGAR", {})["Workers"] = 2
...
def _fake_fetch(self, ticker, *, whitelist, start_expr, progress_fn=None, stop_flag=None, idx=0, total=0):
    ...
    message = (
        ...
        f"ticker={ticker} raw_filings={len(rows)} kept_after_form_filter={len(rows)} duration_ms=1"
    )
    if progress_fn:
        progress_fn(message)
...
def test_weekly_filings_logging_presence(monkeypatch, tmp_path, caplog):
    ...
    combined = progress_messages + logged

    assert any("WEEKLY_FILINGS_TICKER" in msg for msg in combined)
    assert any("WEEKLY_FILINGS_SUMMARY" in msg for msg in combined)
